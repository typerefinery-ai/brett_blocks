...
##############################################################################
# Title: Get Default Incvident and its Objects in a List
# Author: OS-Threat
...
def search_context_memory_for_object(obj_id: str, incident_id: str = None) -> Optional[Dict]:
    ...
    3. All identity directories' JSON files
    4. Company data files (users.json, platforms.json, systems.json)
...
def generate_nodes_and_edges(nodes: List[Dict]) -> tuple[List[Dict], List[Dict]]:
...
def annotate_subgraphs_by_promotables(nodes, layout_options):
	"""
...
def annotate_remaining_nodes_with_positions(nodes, layout_options) -> List[Dict]:
	"""
	Annotate remaining nodes (not connected to promotables) with positionX and positionY.
	...
	for node in nodes:
		...
		currentX += distanceX
		if currentX > left + 4 * distanceX:  # wrap after 4 nodes
			currentX = left
			currentY += distanceY
...
def annotate_incident_nodes_with_positions(incident_nodes) -> List[Dict]:
	...
	with open(TR_Settings_Dir + "/options.json", "r") as mem_input:
		...
		all_promo_nodes = annotate_subgraphs_by_promotables(other_nodes, layout_options)
...
def get_default_incidents_objects():
    ...
    if os.path.exists(TR_Context_Memory_Dir + "/" + context_map):
        with open(TR_Context_Memory_Dir + "/" + context_map, "r") as current_context:
            ...
            current_company = local_map["current_company"]
