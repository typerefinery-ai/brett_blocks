...
test = Sequence()
def make_anecdote(anecdote_form, anecdote_provider=None):
    ...
    for k,v in main:
        contents[k] = main[v]
    for k,v in optional:
        contents[k] = optional[v]
    for k,v in extensions:
        contents["extensions"] = {k, v}
    for k,v in sub:
        pass

    if anecdote_provider:
        contents["provided_by_ref"] = anecdote_provider.get("id")
    if required["modified"] == "":
        # object needs to be created
        stix_obj = Anecdote(contents)
...
def main(inputfile, outputfile):
    ...
    if "provided_by" in input:
        anecdote_provider = input["identity"]


    # setup logger for execution
    stix_dict = make_anecdote(anecdote_form, anecdote_provider)
