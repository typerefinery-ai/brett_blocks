...
def make_task(task_form, changed_objects=None):
    # 1. Extract the components of the object
    required = task_form["base_required"]
    optional = task_form["base_optional"]
    main = task_form["object"]
    extensions = task_form["extensions"]
    sub = task_form["sub"]
    contents = {}
    empties_removed = {}
    ...
    if "modified" in required and required["modified"] == "":
        ...
        stix_dict = Task(**empties_removed)
...
def main(inputfile, outputfile):
    ...
    results = {}
    results["task"] = []
    results["task"].append(json.loads(stix_dict))
