...
:observation_url: https://docs.oasis-open.org/cti/stix/v2.1/os/stix-v2.1-os.html#_p49j1fwoxldc
...
= [stixtitle]*Incident Core Extension Version 3.0 for STIX^TM^ Version 2.1*

[.stix-doc-information-heading]#Draft#

[.stix-doc-information-heading]#01 November 2023#
...
In the 2.0 version of the core incident extension information on impact, events, and tasks were separated into independent SDO's, however this approach did not meet TC requirements and did not collect provenance of observations to enable judgement on evidence.

As such a 3.0 version of this extension has been created in order to improve the sequencing model, to more closely approximate CACO, and to introduce a better method collecting provenance of observations to enable better decision processes, and more complex incidents to be accurately modeled.
...
The properties and additional types within the Incident Core Extension are defined below. As this is not a top-level object, fields such as identifier are not present.  This extension *MUST* use [stixliteral]#extension-definition--ef765651-680c-498d-9894-99799f2fa126# as its extension ID.
The Python class name is [stixliteral]#IncidentCoreExt#.
...
This value *MUST* be between 0 to 100. This can be translated into qualitative values as described in <<appendix-b,Appendix B>>.

|*event_refs* (optional)
|[stixtype]#{list_url}[list]# of type [stixtype]#{identifier_url}[identifier]#
|A list of events tied to this incident.
It *MUST* contain references to one or more Event objects.
...
|*other_object_refs* (required)
|[stixtype]#{list_url}[list]# of type [stixtype]#{identifier_url}[identifier]#
|A list of all SDO, SCO and SRO objects contained in this incident, not including Task, Event, Impact or Sequence objects.
It *MUST* contain references to one or more SDO, SCO or SRO objects.
...
It *MUST* contain references only to Sequence objects.

|*sequence_start_refs* (optional)
|[stixtype]#{list_url}[list]# of type [stixtype]#{identifier_url}[identifier]#
|A list of ence objects tied to this incident.
It *MUST* contain references only to Sequence objects, where the step_type property is set to [stixliteral]#start_step#.

|*task_refs* (optional)
|[stixtype]#{list_url}[list]# of type [stixtype]#{identifier_url}[identifier]#
|A list of tasks tied to this incident.
It *MUST* contain references to one or more Task objects.
...
[[event]]
=== 2.2. Event
The Python class name is [stixliteral]#Event#.
...
This should be drawn from [stixtype]#<<event-type-ov,event-type-ov>>#.
...
|An event has an impact on specific infrastructure.
...
==== 2.2.2 Event Extension Definition
The definition extension  *MUST* use [stixliteral]#extension-definition--4ca6de00-5b0d-45ef-a1dc-ea7279ea910e# as its extension ID.
The Python class name is [stixliteral]#EventCoreExt#.
...
[[impact]]
=== 2.3. Impact
The Python class name is [stixliteral]#Impact#.
...
[[task]]
=== 2.4. Task
The Python class name is [stixliteral]#Task#.
...
==== 2.4.2 Task Extension Definition
The definition extension  *MUST* use [stixliteral]#extension-definition--2074a052-8be4-4932-849e-f5e7798e0030# as its extension ID.
The Python class name is [stixliteral]#TaskCoreExt#.
...
[[evidence]]
=== 2.6. Sighting Extensions for Evidence
The [stixtype]#{sighting_url}[Sighting]# object is used for connecting [stixtype]#{observation_url}[Observed Data]# objects to SDO's, such as Indicators, Malware and Threat Actors.

Observations and Sightings have different evidentiary weightings based on their type (what they are) and provenance (how they were derived).
Weightings cannot be established as provenance data is not collected for each observation, and confidence cannot be established.
At present, Observations and Sightings cannot be added together as evidence, as they are different in nature.


==== 2.6.1 Evidence Extensions

There are at least 8 distinguishable sources of Sightings, each with their own data format for recording their provenance. These are defined as extensions, as listed below.

Producers *SHOULD* use the definition extension and one of the 8 evidence extensions, to attach provenance to a [stixtype]#{sighting_url}[Sighting]# object.


===== 2.6.1.1 Evidence Extension Definition
The definition extension  *MUST* use [stixliteral]#extension-definition--0d76d6d9-16ca-43fd-bd41-4f800ba8fc43# as its extension ID. The Python class name is [stixliteral]#SightingEvidence#.
...
===== 2.6.1.2. Alert Evidence Extension
An Alert can be issued by a system or a user. Generally, when an Alert is issued, it is not known whether it is actually nefarious.
As an example:

- A suspicious email is reported by a user, and the associated SCO’s and SRO’s are collected in an Observed-Data object
- A Sighting is established where the Indicator has a type of “unknown”, and connected to an Event

Alerts are often used to initiailise an incident. Conceivably, once an Alert is qualified as a true positive, it could be joined to another Alert from another Incident. The Python class name is [stixliteral]#SightingAlert#.
...
|*name* (required)
|[stixtype]#{string_url}[string]#
|The name of the Alert as it appears in the alerting system, or  based on the user report

|*log* (optional)
|[stixtype]#{string_url}[string]#
|The content of the Alert as it appears in the alerting system, or  based on the user report

|*system_id* (optional)
|[stixtype]#{string_url}[string]#
|The unique identifier in the origin system

|*source* (optional)
|[stixtype]#{string_url}[string]#
|The origin of the alarm for example McAfee SIEM v12.0

|*product* (optional)
|[stixtype]#{string_url}[string]#
|The product that detected as open vocab for example AV,EDR,IPS,SIEM etc

|*format* (optional)
|[stixtype]#{string_url}[string]#
|An open vocab with known formats like syslog, json, text, LEEF, CEEF etc
...
===== 2.6.1.3. Anecdote Evidence Extension
An Anecdote can be issued only by a person. Generally, when an Anecdote is issued, it is not known whether it is actually nefarious. As an example:

- An Anecdote is established and then collected in an Observed-Data object
- A Sighting is established where the SDO is of type Identity, and then connected to an Event

Anecdotes are used to initiailise an incident. Conceivably, an Anecdote may also be issued that defines an Impact, rather than an Event
Anecdotes are expected to have low confidence and result in the need to do tasks to increase the confidence. The Python class name is [stixliteral]#SightingAnecdote#.
...
|[stixtype]#{string_url}[string]#
|The name of the person who reported the anecdote

|*person_context* (optional)
|[stixtype]#{string_url}[string]#
|(employee, 3rd party, injured, police, anonymous)

|*report_submission* (optional)
|[stixtype]#{string_url}[string]#
|(mail, interview, transcript, telephone)
...
===== 2.6.1.4. Context Evidence Extension
Context data is based on querying of internal systems and has 100% confidence
...
Non-SCO Context data is not an observation, nor a sighting, but it is Evidence. Different Context sources (e.g. SAP, SalesForce etc.) will probably have different provenance formats.  The Python class name is [stixliteral]#SightingContext#.
...
|*name* (required)
|[stixtype]#{string_url}[string]#
|The system the context came from; Outlook, SAP etc.
...
|*value* (optional)
|[stixtype]#{string_url}[string]#
|The value from the origin system

|===

<<<

===== 2.6.1.5. Exclusion List Evidence Extension
An Exclusion List is a source that provides a list of entities that have been shown by their processes (?) to be nefarious. Generally Exclusion lists are focused on specific approaches, such as phshing etc.

There are often multiple sources covering the same approach, and it is to be expected that if an entity is found in multiple sources, then it has more confidence than if it is found in one out of many sources.

A search on an Exclusion List can result in a sighting, but not an event, as it adds to the data available about.  The Python class name is [stixliteral]#SightingExclusion#.
...
===== 2.6.1.6. Enrichment Evidence Extension
An Enrichment is an expansion in the evidence, by leveraging existing data and querying paid or free intel sources to return observables.
Splunk lists common observables as BITCOIN ADDRESSES, CIDR BLOCK, CVE, EMAIL ADDRESS, IP, MALWARE, MD5, REGISTRY KEY, SHA1 and SHA256, SOFTWARE, URL and DOMAIN.

A common OSINT system is Maltego.
The results of an Enrichment will generally be an SCO, although SDO’s can also be returned.  Note observables may not be in the current SCO list.  The Python class name is [stixliteral]#SightingEnrichment#.
...
|*name* (required)
|[stixtype]#{string_url}[string]#
|The site the enrichment came from
...
===== 2.6.1.7. Hunt Evidence Extension
A Hunt is a targeted search using a hunting platform, such as Kestrel, or similar, started as a Task.
...
A Hunt will result in an Event when an SCO is found, but can also result only in an Impact.  The Python class name is [stixliteral]#SightingHunt#.
...
|*name* (required)
|[stixtype]#{string_url}[string]#
|The name of the hunt system

|*playbook_id* (optional)
|[stixtype]#{string_url}[string]#
|The id for for the playbook

|*rule* (optional)
|[stixtype]#{boolean_url}[boolean]#
|The string of the rule

|===

<<<

===== 2.6.1.8. Framework Evidence Extension
Since SCO’s are ephemeral, the desire is to move up the Pyramid of Pain to more abstract characterisations, which are harder for attackers to evade.
Frameworks, such as Mitre ATT&CK and DISARM are commonly used. As an example:

- A suspicious email is found, and associated SCO’s and SRO’s are collected in Observed-Data object
- Once phishing is confirmed, a Sighting is established with ATT&CK Technique T1566

Assignment of a Framework Component requires a judgement, or a correlation from a knowledge base, and thereby has subjectivity.  The Python class name is [stixliteral]#SightingFramework#.
...
|*domain* (optional)
|[stixtype]#{string_url}[string]#
|The domain of the framework

|*comparison* (optional)
|[stixtype]#{string_url}[string]#
|The method used to determine the match

|*comparison_approach* (optional)
|[stixtype]#{string_url}[string]#
|Optional detail on how the comparison between observations and technique were made was made

|===

<<<

===== 2.6.1.9. External Evidence Extension
Threat Reports are posted by researchers on free and paid services, such as MISP. This is one method of confirming that an unknown Indicator is actually malicious. As an example:

- A suspicious email is found, and the associated SCO’s and SRO’s are collected in an Observed-Data object
- A Sighting is established where the Indicator has a type of “malicious-activity”, and connected to an Event

However, reliability of a threat report is a concern, and confidence needs to be established for each individual provider in a channel such as MISP.  The Python class name is [stixliteral]#SightingExternal#.
...
|*pattern* (optional)
|[stixtype]#{string_url}[string]#
|String from a vocabulary (stix, PCRE, sigma, snort, suricata, yara, attckflow)

|*pattern_type* (optional)
|[stixtype]#{string_url}[string]#
|The type of pattern (stix, PCRE, sigma, snort, suricata, yara, attckflow)

|*payload* (optional)
|[stixtype]#{string_url}[string]#
|if payload is simple or linked to external_reference

|*valid_from* (optional)
|[stixtype]#{timestamp_url}[timestamp]#
|Timestamp describing validity
...
Each key *SHOULD* come from [stixtype]#<<entity-type-ov,entity-type-ov>>#.
...
* Brett Forbes, OS-Threat
* Denis O'Sullivan, OS-Threat
* Paolo Di-Prodi, OS-Threat
* Max Barrass, OS-Threat
